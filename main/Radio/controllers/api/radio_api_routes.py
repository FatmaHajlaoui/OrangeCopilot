...
from ...services.mohsen_service import MohsenService
...
def get_congestion_max_daily():
    ...
    if cached_data:
        ...
        time.sleep(2)
        df_max = read_sheet_to_df('CongestionRadio', 'TB croisé max daily',
                                  current_app.config['CREDENTIALS_PATH'], start_at_row=4)
        df_max = df_max.rename(columns={"": "Max"})
...
def get_congestion_max_traffic():
    ...
    if cached_data:
        ...
        # write_df_to_sheet('CongestionRadio', 'export PRS', current_app.config['CREDENTIALS_PATH'], df)
        # time.sleep(2)
        df_traffic = read_sheet_to_df('CongestionRadio', 'Résultat Trafic max par jour ',
                                      current_app.config['CREDENTIALS_PATH'], start_at_row=1)
        df_traffic = df_traffic[~df_traffic.isin(['#N/A']).any(axis=1)]
...
# Battery
@radio_api_bp.route("/optim/data")
def get_mohsen_data_table():
    upload_date = request.args.get('uploadDate')
    if upload_date == "None":
        upload_date = None

    df = MohsenService.get_data(upload_date)
    ...
    export_excel = int(request.args.get('export_excel'))
    if export_excel:
        return save_df_to_excel(df, 'export_optim.xlsx', current_app.config['TEMP_FOLDER'])
