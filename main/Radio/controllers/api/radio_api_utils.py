...
def optim_radio(df2, df1):

    df2 = df2.drop(df2.shape[0] - 1)
    df1 = df1.drop(df1.shape[0] - 1)
    df1['Local Cell ID'] = df1['Cell ID'] + 200000
    data = df1.merge(df2, on=['Time', 'Local Cell ID'])
    data = data.rename(columns={'Cell Name': 'Cell_Name'})
    data = data.rename(columns={'VS.MeanRTWP.Locell.SectorEqm.Ant1(dBm)': 'vs_mean_rtwp_ant0'})
    data = data.rename(columns={'VS.MeanRTWP.Locell.SectorEqm.Ant0(dBm)': 'vs_mean_rtwp_ant1'})

    data = data[['Time', 'Cell_Name', 'VS.MeanRTWP(dBm)', 'vs_mean_rtwp_ant0',
                 'vs_mean_rtwp_ant1']]

    data = data.loc[data['vs_mean_rtwp_ant0'] != 'NIL']
    data = data.loc[data['vs_mean_rtwp_ant1'] != 'NIL']
    data = data.astype(
        {'vs_mean_rtwp_ant0': np.float64, 'vs_mean_rtwp_ant1': np.float64})
    data['Time'] = pd.to_datetime(data['Time'], format='%d-%m-%Y %H:%M')
    data = data.sort_values(['Time'])
    day = (date.today() - timedelta(days=4)).strftime("%Y/%m/%d")
    data_interf = data.loc[data['Time'] >= day]

    def interf(group):
        occ = (group['VS.MeanRTWP(dBm)'] > -98).sum()
        return occ

    data_stat = data_interf.groupby('Cell_Name').apply(lambda x: interf(x[['VS.MeanRTWP(dBm)']])).reset_index(
        name='occ_rtwp_gt_98')

    l = data_stat.loc[data_stat['occ_rtwp>-98'] > 10, 'Cell_Name'].tolist()
    data_interf = data_interf.loc[data_interf['Cell_Name'].isin(l)]
    data_interf['diff'] = np.abs(
        data_interf['vs_mean_rtwp_ant0'] - data_interf['vs_mean_rtwp_ant1'])

    def diff(group):
        occ = (group['diff'] > 5).sum()
        return occ

    data_stat1 = data_interf.groupby('Cell_Name').apply(lambda x: diff(x[['diff']])).reset_index(
        name='occ_rtwp_gt_98')
    data_stat2 = data_interf.groupby('Cell_Name')['VS.MeanRTWP(dBm)'].agg(['mean', 'max', 'min']).reset_index()
    data_stat3 = data_interf.groupby('Cell_Name')[
        ['vs_mean_rtwp_ant0', 'vs_mean_rtwp_ant1']].agg('mean').reset_index()

    data_statf = data_stat2.merge(data_stat, on='Cell_Name')
    data_statf = data_statf.merge(data_stat3, on='Cell_Name')
    data_statf = data_statf.merge(data_stat1, on='Cell_Name')
    data_statf.rename(columns={'mean': 'mean_rtwp', 'max': 'max_rtwp', 'min': 'min_rtwp'}, inplace=True)
    df=data_statf
    print(df)
